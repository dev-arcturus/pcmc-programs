...
while True:
  try:
    ...
    for i in range(len(GT1)):
      if i < 2:
        if GT1[i] not in GT_OPTS[:2]:
          raise GamateError("CHARS")
      elif i < 4:
        if GT1[i] not in GT_OPTS[2:4]:
          raise GamateError("CHARS")
    ...
    for i in range(len(GT2)):
      if i < 2:
        if GT2[i] not in GT_OPTS[:2]:
          raise GamateError("CHARS")
      elif i < 4:
        if GT2[i] not in GT_OPTS[2:4]:
          raise GamateError("CHARS")
...
for x in GTD_MAP.keys():
  if GT_OPTS[0] in x[:2]:
    ...
    a = RP1
  if GT_OPTS[2] in x[2:4]:
    b = DP2
  else:
    b = RP2

  if x == f"{GT_OPTS[0] * 2}{GT_OPTS[2] * 2}" or x == f"{GT_OPTS[1] * 2}{GT_OPTS[3] * 2}":
    c = "Pure"
  else:
    c = "Dominated"
